# Alba Ayala | Frontend Developer Portfolio

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Responsive Design](https://img.shields.io/badge/Responsive-Design-3C8DBC?style=for-the-badge&logo=responsive&logoColor=white)

Una página web de portfolio moderna y minimalista construida con React y Tailwind CSS. Presenta el trabajo y habilidades de Alba Ayala, Frontend Developer.
Link aquí: -

## 🚀 Características del Portfolio

- **Diseño Moderno**: Interfaz limpia y atractiva con tipografía personalizada
- **Totalmente Responsive**: Adaptado a todos los dispositivos (móvil, tablet, desktop)
- **Optimizado para Performance**: Carga rápida y experiencia de usuario fluida
- **Accesibilidad**: Implementa las mejores prácticas de accesibilidad web
- **Animaciones Suaves**: Transiciones y efectos visuales cuidadosamente diseñados

## 🚀 Instalación y Uso

### Prerrequisitos
1. Node.js (versión 16 o superior)
2. npm o yarn

### Pasos para instalar
1. Clonar el repositorio:
```bash
git clone https://github.com/Sailok25/albaayala.git
cd albaayala
